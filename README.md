...
| ~~B11~~ | ~~Medium~~ | **Found and repaired 2026-09-10 (operator report: Gemini answered with "Please run /login").** B07 returned a headless permission denial as 403; Claude Code reads 401 and 403 as an authentication failure and prompted for login on a lane whose OAuth session was fine. Repair: a denial is 400, terminal, shown as an API error, not retried, never mistaken for an expired login. Test: `test/headless-bridge-denial-status.test.ts`; mutation control over the whole suite fails it. The underlying denial (a native `read_file` that headless agy cannot grant) is a separate open item: native read-only tools are not on the allow list of the call-scoped agy settings, so a routed Gemini that reads a file before using the bridged Playwright tools stops the turn. |
