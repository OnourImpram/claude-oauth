...
| G08 | Suspected | The live xAI MCP handshake has not been shown from this tree. |
| ~~N01~~ | ~~High~~ | **Repaired 2026-09-08.** Base64 image tool results cross the shared MCP bridge on both xAI/ACP and Google/Antigravity lanes; text compilation and unsupported/URL media use deterministic type, media-type and byte-size placeholders (URL size is unknown). Adapter-to-MCP tests preserve image bytes and error status; the production-log fixture measures payload-free image metadata. Live provider image interpretation is NOT_RUN, no vision task was submitted. |
