...
| G04, G08 | High / suspected | `thinking`/`image`/`document` blocks in history are rejected with 422 on the agent lanes; the last text block of a multi-block user message is taken as "the request"; the live xAI MCP handshake has not been shown from this tree. Delegate tool restrictions are repaired under N03. |
